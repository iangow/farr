...
get_event_cum_rets <- function(data, conn,
    ...
    crsp.dsf <- tbl(conn, sql("SELECT * FROM crsp.dsf"))
    crsp.erdport1 <- tbl(conn, sql("SELECT * FROM crsp.erdport1"))
    crsp.dsi <- tbl(conn, sql("SELECT * FROM crsp.dsi"))
    ...
    dsf_plus <-
        ...
        dplyr::mutate(ret = (1 + dplyr::coalesce(ret, 0)) * (1 + dplyr::coalesce(dlret, 0)) - 1) %>%
    ...
    results <-
        ...
        dplyr::group_by(permno, event_date, end_event_date) %>%
        dplyr::summarize(ret_raw =
                      exp(sum(ln((1 + ret)), na.rm = TRUE)) - 1,
                  ret_mkt =
                      exp(sum(ln((1 + ret)), na.rm = TRUE)) -
                      exp(sum(ln((1 + vwretd)), na.rm = TRUE)) ,
                  ret_sz =
                      exp(sum(ln((1 + ret)), na.rm = TRUE)) -
                      exp(sum(ln((1 + decret)), na.rm = TRUE)),
                  .groups = "drop")
